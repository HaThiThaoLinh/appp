    ...
    if ($stmt->rowCount() > 0) {
        ...
        if ($stmt->execute()) {
            ...
            try {
                ...
                $mail->SMTPAuth = true;
                $mail->Username = 'your_email@gmail.com'; // Thay bằng Gmail của bạn
                $mail->Password = 'your_app_password'; // Thay bằng mật khẩu ứng dụng hoặc mật khẩu tài khoản
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
                $mail->Port = 587;

                $mail->setFrom('your_email@gmail.com', 'Nhom6'); // Thay bằng Gmail của bạn
                $mail->addAddress($email); 

                $mail->isHTML(true);
                $mail->Subject = 'Xác nhận đăng ký tài khoản';
                $mail->Body = "Click vào liên kết sau để xác nhận tài khoản của bạn: 
                <a href='http://localhost:8888/send_email/verify.php?token=$token'>Xác nhận tài khoản</a>";
